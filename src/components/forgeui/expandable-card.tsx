...
export default function ExpandableCard({
    ...
    const ref = useOutsideClick(() => setCurrent(null), current !== null)
            ...
            <AnimatePresence>
                {current ? (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="bg-opacity-10 pointer-events-none absolute inset-0 z-10 bg-background/50 backdrop-blur-xl"
                    />
                ) : null}
            </AnimatePresence>
                    ...
                    <div className="absolute inset-0 z-10 grid place-items-center">
                        ...
                        >
                            <div className="flex w-full items-center gap-4">
                                <motion.div
                                    layoutId={`cardItemIcon-${current.id}`}
                                >
                                    {current.icon}
                                </motion.div>
                ...
                <div className="relative flex w-full flex-col items-center gap-4 px-2">
                    {items.map((item) => (
                        <motion.div
                            ...
                            className="flex w-full cursor-pointer flex-row items-center gap-4 rounded-sm bg-background p-2 shadow-sm ring-1 shadow-black/10 ring-black/10 md:p-4 dark:ring-neutral-800/60"
                            onClick={() => {
                                setCurrent(item)
                            }}
                        >
